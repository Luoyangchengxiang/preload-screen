...
describe('LifecycleManager', () => {
  ...
  it('构造时记录创建时间', () => {
    ...
    const lm = new LifecycleManager(mockConfig(), mockDom, mockAnimation);
  ...
  describe('auto 模式 & MutationObserver', () => {
    it('元素初始存在，立即绑定 Observer', () => {
      ...
      vi.stubGlobal('MutationObserver', vi.fn(() => ({ observe: observeSpy, disconnect: disconnectSpy })));

      new LifecycleManager(mockConfig(), mockDom, mockAnimation);
    ...
    it('元素初始不存在，轮询 50ms/次，找到后绑定 Observer', () => {
      const observeSpy = vi.fn();
      vi.stubGlobal('MutationObserver', vi.fn(() => ({ observe: observeSpy, disconnect: vi.fn() })));
    ...
    it('轮询 100 次仍未找到则放弃（debug 时打印警告）', () => {
      ...
      new LifecycleManager(config, mockDom, mockAnimation);
  ...
  describe('beforeHide & 最小展示时间', () => {
    it('已到达最小展示时间，立即调用 hide', () => {
      const lm = new LifecycleManager(mockConfig({ minShow: 0 }), mockDom, mockAnimation);
    ...
    it('未到达最小展示时间，延时 hide', () => {
      const lm = new LifecycleManager(mockConfig({ minShow: 300 }), mockDom, mockAnimation);
  ...
  describe('hide', () => {
    it('调用 dom.fadeOutAndRemove 并传入配置 fadeOut 时长', () => {
      const lm = new LifecycleManager(mockConfig({ fadeOut: 888 }), mockDom, mockAnimation);
      (lm as any).hide();
